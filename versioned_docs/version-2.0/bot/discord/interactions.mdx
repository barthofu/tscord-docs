...
TSCord is [defering](https://discordjs.guide/slash-commands/response-methods.html#deferred-responses) every command interaction (slashes and context menus) by default, so you should use `.followUp()` instead of `.reply()`.

If you want to get rid of this automatic defering, you can delete the following lines in the `src/events/interactionCreate`:
```ts 
// defer the reply
