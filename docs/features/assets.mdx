...
The uploaded images url are then saved in the `image` table of the database, along with some usefull metadata.
...
Plus, if an image is removed from the directory, it'll also be removed from both database and imgur, and if one has a broken link on imgur, it'll be automaticaly reuploaded!
...
export default class PingCommand {
    ...
    async pingHandler(
        ...
        //highlight-next-line
        const image = await this.db.get(Image).find({ fileName: 'image.png' })
