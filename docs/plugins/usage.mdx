...
```ts
import * as puginServices from '@services/{pluginName}'
``` 
where `{pluginName}` is the name of the plugin folder.

### Translations

There's a high probabilty that a plugin you want to use do not support all the same languages than your bot, so you can't add and edit translation for each locales. <br/>
To do that:
1. First run the bot in `dev` mode after installed the plugin so the trnaslation will be generated inside `/src/i18n/`.
