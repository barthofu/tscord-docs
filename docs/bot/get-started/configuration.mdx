...
If your IDE displays you errors in certains files, it is because its TypeScript Server is not using the right version.

### VSCode

For VSCode, follow these steps:

1. Go in a `.ts` file (any)
2. Open the command palette (`ctrl`+`maj`+`p`)
3. Search: ***Select Typescript*** and click on the first result
